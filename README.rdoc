...
First is <tt>:options</tt>. These are the <tt>options</tt> section of <tt>/etc/syslog-ng/syslog-ng.conf</tt>. If you don't specify anything, the Ubuntu defaults are used.

Second is <tt>:extra</tt>. This is an extra configuration to append to the end of <tt>/etc/syslog-ng/syslog-ng.conf</tt>.
...
Note that the value of the <tt>:extra</tt> key is appended to the end of <tt>/etc/syslog-ng/syslog-ng.conf</tt>, not within syslog-ng's <tt>options</tt> block like the other configurable parameters shown.
...
This will save rails logs to <tt>/var/log/rails/rails-application-environment.log</tt> and passenger logs to <tt>/var/log/rails/passenger.log</tt>.

If you need log rotation, you can use <tt>logrotate</tt> to handle this for you.

You need to provide additional configuration with the IP address of the log server and a port to listen on:

    # moonshine.yml, note this isn't under the 'options' namespace
    :syslog_ng:
      :log_server_ip: 555.555.555.555
      :log_server_port: 515

This sets some default <tt>options</tt> and applies this plugin's <tt>log-server-syslog-ng</tt> template as the value of <tt>:extra</tt>. If you need more customization, it probably would make sense to create a new template based on it with whatever options and extra configuration you need.
...
This sets some default <tt>options</tt> and applies this plugin's <tt>rails-client-syslog-ng</tt> template as the value of <tt>:extra</tt>. If you need more customization, it probably would make sense to create a new template based on it with whatever options and extra configuration you need.
